// src/features/projects/pages/ProjectSelection.tsx
import { useState } from 'react';
import { useNavigate } from 'react-router-dom';
import { ArrowUpRight, Sparkles, ChevronDown, Loader2 } from 'lucide-react';
import { toast } from 'react-toastify';

import { useProjectPackages as usePackages } from '@/features/projects/hooks/useProjectData';
...
const projectSteps = [
  { id: 1, name: 'Package Selection', description: 'Choose your package' },
  { id: 2, name: 'Add-ons', description: 'Customize with add-ons' },
];
...
const ProjectSelection = () => {
  ...
  // When true, we render the add‑on selection step.
  ...
  const handlePackageSelect = async (pkg: PackageData) => {
    setIsCreating(true);
    try {
      // Create a new project in the projects app using the package type.
      ...
      // Update the selected package with the real project ID.
      const updatedPackage = { ...pkg, projectId: projectResponse.id };
  ...
  return (
    ...
    <StepNavigation steps={projectSteps} currentStep={1} type="project" />
